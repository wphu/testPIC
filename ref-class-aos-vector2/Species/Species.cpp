...
Species::Species(Parameters& params)
{
    ...
    for(int i_space = 0; i_space < params.n_space; i_space++)
    {
        for(int i_particle = 0; i_particle < particles[i_space].size(); i_particle++)
        {
            ...
            particles[i_space][i_particle].x = ((double)rand() / RAND_MAX) * dx;
...
void Species::interpolator1D1Order(double *Ex, int i_space, int i_particle, LocalFields &E_local)
{
    ...
    xjn = particles[i_space][i_particle].x * dx_inv;
...
void Species::pusherBoris(int i_space, int i_particle, LocalFields &E_local)
{
    ...
    particles[i_space][i_particle].x  += particles[i_space][i_particle].vx * dt;
}
...
void Species::pusherBoris(int i_space, int i_particle, LocalFields &E_local, LocalFields &B_local)
{
    ...
    // Move the particle
    particles[i_space][i_particle].x += dt * particles[i_space][i_particle].vx;
}
...
//cout<<"aaaa "<<endl;
    for(int i_space = 0; i_space < particles.size(); i_space++)
    {
        for(int i_particle = 0; i_particle < particles[i_space].size(); i_particle++)
        {
            i_space_change =  floor(particles[i_space][i_particle].x * dx_inv);
            if(i_space + i_space_change < 0 || i_space + i_space_change >= particles.size())
            {
                //cout<<"aaa "<<i_space<<endl;
                //cout<<"aaa "<<i_space_change<<endl;
                //cout<<"aaa "<<particles[i_space][i_particle].vx<<endl;
            }
            if(i_space + i_space_change < 0)
            {
                particles[i_space][i_particle].x = -particles[i_space][i_particle].x - 2.0 * i_space * dx;
            }
            else if(i_space + i_space_change >= particles.size())
            {
                particles[i_space][i_particle].x = -particles[i_space][i_particle].x - 2.0 * (i_space * dx - lx);
            }
        }
    }
...
//cout<<"ccc "<<endl;
    for(int i_space = 0; i_space < particles.size(); i_space++)
    {
        ...
        for(int i_particle = 0; i_particle < particles[i_space].size(); i_particle++)
        {
            ...
            if(i_space + i_space_change < 0 || i_space + i_space_change >= particles.size())
            {
                cout<<"ccc "<<i_space<<endl;
                cout<<"ccc "<<i_space_change<<endl;
                cout<<"ccc "<<particles[i_space][i_particle].x<<endl;
            }
            */
            if(i_space_change != 0)
            {
                particles[i_space][i_particle].x = abs(particles[i_space][i_particle].x - i_space_change * dx);
...
//cout<<"ddd "<<endl;    
    for(int i_space = 0; i_space < particles.size(); i_space++)
    {
        for(int i_list = 0; i_list < list_removed_particles[i_space].size(); i_list++)
        {
            ...
            //particle_temp =  particles[i_space][i_particle];
            //particles[i_space][i_particle] = particles[i_space][particles[i_space].size() - i_list];
            //particles[i_space][particles[i_space].size() - i_list] = particle_temp;
            swap(particles[i_space][i_particle], particles[i_space].back());
            particles[i_space].pop_back();
        }
...
void Species::sort_particles_new()
{
    int i_space_change;
    int n_space_check_boundary;
